export function addSearchTabToNav(onSearchClick: () => void) {
  const exploreNav = document.querySelector('.explore__nav');
  if (!exploreNav) return;

  let searchTab = document.querySelector('button[data-search-tab="true"]') as HTMLButtonElement | null;
  
  if (!searchTab) {
    searchTab = document.createElement('button');
    searchTab.className = 'explore__nav-component';
    searchTab.setAttribute('data-search-tab', 'true');
    (searchTab as HTMLButtonElement).style.cssText = 'background: none; border: none; cursor: pointer; display: flex; align-items: center; gap: 8px; color: inherit; font-size: inherit; padding: 6px 8px; transition: all 0.15s ease; box-sizing: border-box;';
    searchTab.innerHTML = `
    ...
    exploreNav.appendChild(searchTab);
  }
  
  searchTab.addEventListener('click', (e: Event) => {
    e.preventDefault();
    e.stopPropagation();
    document.querySelectorAll('.explore__nav-component').forEach(tab => {
      (tab as HTMLElement).classList.remove('selected');
    });
...
export function modifyExplorePage() {
  const exploreNav = document.querySelector('.explore__nav');
  if (!exploreNav) return;
  ...
  const existingSearchContainer = document.getElementById('cheffs-explore-search-container');
  if (existingSearchContainer) {
    existingSearchContainer.remove();
  }
}

export function handleExploreTabRedirect() {
  const currentUrl = window.location.href;
  const urlWithoutHash = currentUrl.split('#')[0];
  
  if (urlWithoutHash.includes('/explore/')) {
    const exploreMatch = urlWithoutHash.match(/\/explore\/[^/]+/);
    if (exploreMatch) {
      localStorage.setItem('cheffs_auto_load_search', 'true');
      window.location.href = 'https://flavortown.hackclub.com/explore';
      return true;
    }
  }
  return false;
...
export function hideExploreContent() {
  const exploreContent = document.querySelector('.explore__content') as HTMLElement | null;
  if (exploreContent) {
  ...
  const loadMoreBtn = document.querySelector('[data-load-more-target="button"]');
  if (loadMoreBtn) {
    loadMoreBtn.remove();
  }
}
